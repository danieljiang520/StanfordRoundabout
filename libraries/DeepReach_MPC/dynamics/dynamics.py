...
#Extra things for the TwoCar8D dynamics class
...
#             self.delta_max,
#             max(abs(self.a_min), abs(self.a_max)),
#         ]).cuda()
...
#         dsdt[..., 2] = state[..., 3] / self.L * torch.tan(delta1)
#         dsdt[..., 3] = a1

#         dsdt[..., 4] = state[..., 7] * torch.cos(state[..., 6])
#         dsdt[..., 5] = state[..., 7] * torch.sin(state[..., 6])
#         dsdt[..., 6] = state[..., 7] / self.L * torch.tan(delta2)
#         dsdt[..., 7] = a2
...
#         dsdt_ = self.dsdt(state, opt_control, opt_disturbance)
...
#         c_delta1 = dvds[..., 2] * state[..., 3] / self.L
#         c_a1 = dvds[..., 3]

#         u[..., 0] = self.delta_max * torch.sign(c_delta1)
#         u[..., 1] = torch.where(c_a1 >= 0, self.a_max, self.a_min)

#         return self.clamp_control(state, u)

#     def optimal_disturbance(self, state, dvds):
#         batch_dims = state.shape[:-1]
#         d = torch.zeros(*batch_dims, 2, device=state.device)

#         c_delta2 = dvds[..., 6] * state[..., 7] / self.L
#         c_a2 = dvds[..., 7]

#         d[..., 0] = -self.delta_max * torch.sign(c_delta2)
#         d[..., 1] = torch.where(c_a2 >= 0, self.a_min, self.a_max)

#         return self.clamp_disturbance(state, d)
